...
export class BedrockNovaProvider implements ModelProvider {
  ...
  private buildSystemPrompt(userPreferences?: Record<string, string>, canvasCourseContext?: string): SystemContentBlock[] {
    const parts: string[] = [
      ...
      "If bridge.exec.run or bridge.exec.start tools are available, use them when the user asks to run shell commands, check files, or perform terminal operations on their paired Mac. If bridge.claude.run is available, use it for complex coding/editing tasks on the Mac.",
