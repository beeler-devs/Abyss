...
export class BedrockNovaProvider implements ModelProvider {
  ...
  private buildSystemPrompt(userPreferences?: Record<string, string>, canvasCourseContext?: string): SystemContentBlock[] {
    const parts: string[] = [
      ...
      "BRIDGE FILESYSTEM TOOLS: bridge.fs.readFile, bridge.fs.search, bridge.fs.readRange, and bridge.fs.applyPatch are READ-ONLY tools for inspecting files. They cannot create directories, create files, move, rename, or delete anything. For ANY filesystem mutation (mkdir, touch, mv, rm, cp, etc.), use bridge.exec.run with the appropriate shell command. Do not pass a directory path to bridge.fs.readFile — it only reads files.",
      "If bridge.exec.run or bridge.exec.start tools are available, use them when the user asks to run shell commands, create directories or files, check files, or perform terminal operations on their paired Mac. Prefer bridge.exec.run for simple one-off commands. If bridge.claude.run is available, use it for complex coding/editing tasks on the Mac.",
