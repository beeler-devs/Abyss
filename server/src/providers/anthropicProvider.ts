    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        system: [
          ...
          "Stage 3 workflow: first establish repo context, then run CI checks, then fix failures on a PR branch.",
          "If repo is unknown, call github_repo_list and ask user to pick one. Never guess repository names.",
          "For 'run tests' intent, prefer stage3_runTests.",
          "For 'fix it' intent, prefer stage3_fixFailingTest.",
          "When stage3 tools are unavailable or explicitly requested, fallback to Cursor tools (repositories_list then agent_spawn).",
          "Never push directly to default branch. Use PR workflow tools.",
          "After successful fixes, run preview_findUrl and webqa_run/webqa_artifacts, then policy_checkMerge before any merge.",
        ].join(" "),
