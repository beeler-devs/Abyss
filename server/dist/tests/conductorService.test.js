...
class ToolCaptureProvider {
    name = "tool-capture";
    toolNames = [];
    async generateResponse(_conversation, tools) {
...
test("spawn tool.result maps agentId to session and emits agent.status", async () => {
    ...
    const statusEvent = emitted.find((event) => (event.type === "agent.status" && event.payload.agentId === "agent-map-1"));
...
test("webhook FINISHED routes to session and triggers agent.completed flow", async () => {
    ...
    const statusEvent = emitted.find((event) => (event.type === "agent.status"
...
test("deterministic 'run tests' intent invokes bridge.exec.run", async () => {
    ...
    const hasFinalSpeech = emitted.some((event) => (event.type === "assistant.speech.final"
