...
test("bridge tool routing returns timeout without forcing offline status", async () => {
    ...
    const offlineStatusEvent = emitted.find((event) => (event.type === "bridge.status" && event.payload.status === "offline"));
...
test("bridge.claude.run verbose mode logs parsed tool_use progress", async () => {
    ...
    const narratedProgress = emitted.some((event) => (event.type === "tool.call"
    ...
    assert.equal(narratedProgress, true, `expected parsed tool_use narration, emitted=${JSON.stringify(emitted)} logs=${JSON.stringify(logs.entries.map((entry) => entry.message))}`);
    assert.ok(logs.entries.some((entry) => entry.message.includes("bridge.claude.run.progress")), `expected progress log, got: ${logs.entries.map((entry) => entry.message).join(" | ")}`);
});
...
test("bridge.claude.run flushes a final partial stream-json line on finish", async () => {
    ...
    assert.equal(result.error, null);
    assert.ok(emitted.some((event) => (event.type === "tool.call"
        && event.payload.name === "tts.speak"
        && typeof event.payload.arguments === "string"
        && event.payload.arguments.includes("Running terminal command: npm test"))));
});
