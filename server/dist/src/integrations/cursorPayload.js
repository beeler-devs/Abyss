...
export function parseCursorAgentSnapshot(value) {
    ...
    const modeValue = normalizeMode(readString(metadata, ["mode"])
        ?? readString(context, ["mode"])
        ?? readString(root, ["mode"]));
    return {
        agentId,
        status: normalizeStatus(readString(context, ["status"])
...
export function parseCursorAgentSnapshotFromResult(resultPayload) {
    return parseCursorAgentSnapshot(parseJSONRecord(resultPayload));
}
export function parseCursorWebhookPayload(value) {
...
function readString(root, ...paths) {
